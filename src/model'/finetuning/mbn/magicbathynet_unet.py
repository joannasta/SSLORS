...
class UNet_bathy(nn.Module):
    ...
    def forward(self, x, image):
        image = image.float()  # Already normalized by dataset
        batch_size, channels, features = x.shape  # (7, 13, 768)
        target_height = 32
        target_width = features // target_height  # Ensure compatibility
        ...
        # Encoder path with normalized image
        x1 = self.encoder[0](image) 
        x2 = self.encoder[1](x1)
        x3 = self.encoder[2](x2)
        x4 = self.encoder[3](x3)
        
        
        # Combine MAE features with encoder features
        #x4 = x  # or use concatenation: torch.cat([x4, x], dim=1)
        ...
        return output  # Will be in same scale as normalized target'''
