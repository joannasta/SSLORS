...
class MAEFineTuning(pl.LightningModule):
    ...
    '''def training_step(self, batch, batch_idx):
        ...
        target_mask = self._generate_mask(target, batch_size, device)
        data_mask = self._generate_mask(data, batch_size, device, average_channels=True)

        combined_mask = (target_mask * data_mask >= 0.5).float()

        print("data min max mean:", data.min(), data.max(), data.mean())
        ...
        loss = self.criterion(prediction, target, combined_mask)

        pred_np, target_np, combined_mask_np = (
            prediction.detach().cpu().numpy(),
            target.cpu().numpy(),
            combined_mask.cpu().numpy(),
        )

        masked_pred = pred_np * combined_mask_np
        masked_target = target_np * combined_mask_np
        ...
        rmse, mae, std_dev = calculate_metrics(masked_pred[0].ravel(), masked_target[0].ravel())
        rmse = -self.norm_param_depth * rmse
    ...
    '''def validation_step(self, batch, batch_idx):
        ...
        if batch_idx > 0:
            raise ValueError(f"More than one batch processed: batch_idx={batch_idx}")
        ...
        data = torch.clamp(data, min=0, max=1)
        prediction = self(data)
        val_loss = self.criterion(prediction, target, combined_mask)
        pred_np, target_np, combined_mask_np = (
            prediction.detach().cpu().numpy(),
            target.cpu().numpy(),
            combined_mask.cpu().numpy(),
    ...
    def test_step(self, batch, batch_idx):
        ...
        target_eroded = np.pad(target_eroded, ((pad_size, pad_size), (pad_size, pad_size)), mode='reflect')
        ...
        target_eroded = target_eroded[pad_size:-pad_size, pad_size:-pad_size]

        #Generate mask for non-annotated pixels in depth data 
        target_mask = (target_eroded != 0).astype(np.float32) 
        target_mask = torch.from_numpy(target_mask) 
        ...
        masked_pred = pred * combined_mask.cpu().numpy()
        masked_gt_e = target_eroded * combined_mask.cpu().numpy()

        if batch_idx % 100 == 0:
            self.log_images(
                data[0].cpu(),  # First image in batch
                masked_pred[0],  # First prediction in batch
                depth[0].cpu()  # First target in batch
            )

        rmse, mae, std_dev = calculate_metrics(
            masked_pred[0].ravel(), 
            masked_gt_e[0].ravel()
        )
        rmse = -self.norm_param_depth * rmse
        ...
        self.log('test_rmse', rmse * -self.norm_param_depth)
    ...
    def log_images(self, original_images: torch.Tensor, reconstructed_images: torch.Tensor, depth: torch.Tensor) -> None:
        self.log_results()
        ...
        # First subplot: Normalized Prediction
        plt.subplot(1, 2, 1)  # 1 row, 2 columns, first plot
        plt.hist(pred_normalized.flatten(), bins=50, alpha=0.7, color="blue")
        plt.title("Pixel Distribution (Normalized Prediction)")
        plt.xlabel("Pixel Value")
        plt.ylabel("Frequency")

        # Second subplot: Denormalized Prediction
        plt.subplot(1, 2, 2)  # 1 row, 2 columns, second plot
        plt.hist(pred_denormalized.flatten(), bins=50, alpha=0.7, color="green")
        plt.title("Pixel Distribution (Denormalized Prediction)")
        plt.xlabel("Pixel Value")
        plt.ylabel("Frequency")

        # Save the figure
        save_path = os.path.join(self.run_dir, f"pixel_distribution_pred_epoch_{self.current_epoch}.png")
        ...
        # First subplot: Normalized Prediction
        plt.subplot(1, 2, 1)  # 1 row, 2 columns, first plot
        plt.hist(depth.flatten(), bins=50, alpha=0.7, color="blue")
        plt.title("Pixel Distribution (Normalized Depth)")
        plt.xlabel("Pixel Value")
        plt.ylabel("Frequency")

        # Second subplot: Denormalized Prediction
        plt.subplot(1, 2, 2)  # 1 row, 2 columns, second plot
        plt.hist(depth_denorm.flatten(), bins=50, alpha=0.7, color="green")
        plt.title("Pixel Distribution (Denormalized Depth)")
        plt.xlabel("Pixel Value")
        plt.ylabel("Frequency")

        # Save the figure
        save_path = os.path.join(self.run_dir, f"pixel_distribution_depth_epoch_{self.current_epoch}.png")
        plt.tight_layout()  # Adjust spacing between subplots
        plt.savefig(save_path)
        plt.close()
